...
class GtpConnection():
    ...
    def policy_moves_cmd(self, args): ####### get_move_prob, I think need to be in the other py file
        """
        ...
        if len(policy_moves) == 0:
            ...
            result = TreeNode.return_moves_prob(TreeNode(self), self.board, policy_moves, self.board.current_player)
    ...
    def genmove_cmd(self, args):
        ...
        try:
            ...
            move = self.go_engine.get_move(self.board, color, self.go_engine.pattern, self.go_engine.selfatari)
            if move is None:
                self.respond("pass")
                return
